	...
	SubShader
	{
		...
		Pass
		{
			...
			float4 FragmentMain(VertexToFragment input) : COLOR
			{
				...
				float3 finalColor = lerp(lerp(_Color.rgb, diffuseColor.rgb, diffuseColor.a), reflectionColor, _ReflectionStrength);
				...
				return float4(finalColor * matCapColor * 2.0, _Color.a);
			}
