...
public class GameManagerAndUI : MonoBehaviour
{
	...
	public GameObject MobileMovementButtons;
	public GameObject CarChoiceButtons;
	...
	void Start()
	{
		...
		CarChoiceButtons.SetActive(false);
		IsNearCar = false;
	}
	...
	void Update()
	{
		...
		if (ShowInteractText == true)
		{
			...
			{
				InteractText.text = "Take a ride?";
				EnableChoosingCarButtons();
			}
			if (PlayerCharacter.ToEnterVehicle == false && PlayerCharacter.ToPickupWeapon == false)
			{
				InteractText.text = "";
				//DisableChoosingCarButtons();
			}
		}
	...
	public void EnableChoosingCarButtons()
	{
		if (!IsNearCar)
		{
			MobileMovementButtons.SetActive(false);
			CarChoiceButtons.SetActive(true);
			Debug.Log("Near car");
			PlayerCharacter.StopCharacterMovement();
		}
	}

	public void DisableChoosingCarButtons()
	{
		MobileMovementButtons.SetActive(true);
		CarChoiceButtons.SetActive(false);
		Debug.Log("Back");
		PlayerCharacter.ResumeCharacterMovement();
		IsNearCar = false;
	}
}
